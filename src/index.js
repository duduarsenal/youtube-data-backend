...
app.get('/api/videos', async (req, res) => {
    try {
        ...
        const mapeando = infosVideo.map(async (info) => {
            try {
                const urlResponse = await fetch(`https://youtube.googleapis.com/youtube/v3/videos?part=statistics&key=${api_key}&id=${info.resourceId.videoId}`)
                const response = await urlResponse.json()
    
                const {viewCount, likeCount, commentCount} = response.items[0].statistics
